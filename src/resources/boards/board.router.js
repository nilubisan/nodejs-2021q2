...
router.route('/:boardId/tasks').get( async (req,res) => {
  ...
  router.route('/:boardId/tasks/:taskId').get( async (req,res) => {
    const task = await tasksService.getTask(req.params.boardId, req.params.taskId);
    if(!task) {
  ...
  router.route('/:boardId/tasks/:taskId').put( async (req,res) => {
    const task = await tasksService.updateTask(req.body, req.params.boardId, req.params.taskId);
    if(!task){
      res.status(400).send('Bad request');
    } 
    else {
  ...
  router.route('/:boardId/tasks/:taskId').delete(async(req,res) => {
    const result = await tasksService.deleteTask(req.params.boardId, req.params.taskId);
    if(!result) {
