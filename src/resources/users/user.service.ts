...
export const getAllUsersService = async (): Promise<Array<User>> =>
  getAllUsers();

export const getUserByIdService = async (
  userID: string
): Promise<User | 'NOT FOUND'> => getUserByID(userID);

export const userExistsService = async (
  login: string,
  password: string
): Promise<User | null> => userExists(login, password);
...
export const updateUserService = async (
  userID: string,
  user: User
): Promise<User | 'NOT FOUND'> => updateUser(userID, user);
